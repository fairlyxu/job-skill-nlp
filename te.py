...
filename = './data/jobroles_mini.csv'
...
result = cal_matrixSimilarityByLSI(BASE)
print(result)

'''
sim_all.append(cal_matrixSimilarityByLSI(i) for i in range(LENGTH))
# print(type(sim_all))
print(sim_all)

print('--------------------我是分割线---------------------')
tt = np.array(sim_all)
labels = spectral_clustering(tt, n_clusters=CLUSTER_NUM)
print(labels)
data.insert(0, 'clustering', labels)
data.to_csv('./data/output_' + str(CLUSTER_NUM) + '.csv',
            columns=['clustering', 'jobRoleName', 'jobRoleDesc', 'jobCategoryId'], index=False, header=False)
...
f = open(r'cluster_' + str(CLUSTER_NUM) + '.csv', 'a')
